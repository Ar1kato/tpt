...
class Bakery:
    ...
    def add_recipe(self, name: str):
        """Add recipe."""
        complexity = abs(len(name) * len(self.bakers) - (self.min_experience_level))
        self.recipes[name] = complexity
        self.budget = self.budget - abs(len(name))

    def make_order(self, name: str) -> Pastry:
        """Make order."""
        complexity = abs(len(name) * len(self.bakers) - (self.min_experience_level))
        for baker in self.bakers:
            if complexity <= baker.experience_level:
